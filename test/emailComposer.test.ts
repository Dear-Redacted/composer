...
describe("openGmailCompose", () => {
  it("uses the Electron bridge when available", async () => {
    const openGmailCompose_mock = vi
      .fn()
    ...
    await expect(
      openGmailCompose("person@example.com", "Dear note"),
    ).resolves.toBeUndefined();
    expect(openGmailCompose_mock).toHaveBeenCalledWith(
      "person@example.com",
      "Dear note",
    );
  });
  ...
  it("throws when the Electron bridge reports a failure", async () => {
    (window as any).redactedComposer = {
      openGmailCompose: vi
        .fn()
        .mockResolvedValue({ success: false, error: "blocked" }),
    };

    await expect(
      openGmailCompose("person@example.com", "Dear note"),
    ).rejects.toThrow("Failed to open Gmail: blocked");
  });
  ...
  it("uses the fallback error message when the Electron bridge fails without a message", async () => {
    ...
    await expect(
      openGmailCompose("person@example.com", "Dear note"),
    ).rejects.toThrow("Failed to open Gmail: Unknown error");
  });
  ...
  it("throws when the Electron bridge is missing", async () => {
    ...
    await expect(
      openGmailCompose("person@example.com", "Dear note"),
    ).rejects.toThrow("Electron bridge not available");
  });
